...
export default async function DashboardPage() {
  ...
  const [gym, activeMembers, monthRevenue, todayCheckins, expiringSoon, recentMembers, recentPayments, payments14d] =
    await Promise.all([
  ...
  return (
    <div className="space-y-6 max-w-full">
      ...
      <div className="flex flex-col justify-between gap-3 border-b border-[#E5D9C5] pb-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-xs text-[#8C7A6B] mt-0.5">
            Real-time telemetry and management console for {gym?.name}.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/attendance">
            <Button size="sm" variant="default" className="h-8.5 rounded-xl text-xs shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <StatCard
          label="Monthly Run-Rate (MTD)"
          value={formatCurrency(monthRevenue._sum.totalAmount ?? 0)}
          icon={IndianRupee}
          hint="Reconciled collections this month"
        />
        <StatCard
          label="Floor Occupancy Rate"
          value={`${attendancePct}%`}
          icon={Activity}
          hint={`${todayCheckins} of ${activeMembers} active athletes`}
        />
        <StatCard
          label="Renewal Radar (7 Days)"
          value={expiringSoon.length}
          icon={Timer}
          hint="Queued for WhatsApp renewal nudge"
        />
      </div>
          ...
          <div className="mb-4 flex items-center justify-between border-b border-[#E5D9C5] pb-3">
            <div>
              <h3 className="font-mono text-xs font-bold uppercase tracking-widest text-[#8B5E34]">
                Athlete Retention &amp; Health Radar
              </h3>
              <p className="text-[11px] text-[#8C7A6B] mt-0.5">Automated membership retention status</p>
            </div>
            <Badge variant="default" className="text-[10px] py-0.5">Active Radar</Badge>
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-[#E5D9C5] bg-[#F9F8F6] p-4">
              <p className="text-[10px] font-mono font-bold uppercase tracking-widest text-[#8C7A6B]">
                Engagement Index
              </p>
              <p className="mt-1 font-mono text-xl font-bold text-[#33281E]">
                {activeMembers > 0 ? "94.2%" : "100%"}
              </p>
              <p className="text-[10px] text-[#8C7A6B] mt-0.5">Facility check-in frequency</p>
            </div>
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4">
              ...
              <p className="mt-1 font-mono text-xl font-bold text-amber-800">
                {expiringSoon.length} Athletes
              </p>
              <p className="text-[10px] text-[#8C7A6B] mt-0.5">Next 7 days validity window</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-[#E5D9C5] bg-white p-6 shadow-[0_4px_20px_rgba(51,40,30,0.03)]">
          <div className="mb-4 flex items-center justify-between border-b border-[#E5D9C5] pb-3">
            <div>
              ...
              <p className="text-[11px] text-[#8C7A6B] mt-0.5">Latest verified payment inflows</p>
            </div>
            <Link
              href="/payments"
              className="inline-flex items-center gap-1 text-xs font-semibold text-[#8B5E34] hover:underline transition-colors"
          ...
          {recentPayments.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-xs text-[#8C7A6B]">No payment transactions recorded yet.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {recentPayments.map((p) => (
                <div
                  key={p.id}
                  className="flex items-center justify-between rounded-2xl border border-[#E5D9C5] bg-[#F9F8F6] px-3.5 py-2.5 transition-colors hover:border-[#8B5E34]"
                >
                  <div className="min-w-0">
                    <p className="truncate text-xs font-bold text-[#33281E]">
                      {p.member.user.name}
                    </p>
                    <p className="truncate font-mono text-[10px] text-[#8C7A6B]">
                      {p.method.replace("_", " ")} · {p.paidAt ? formatDate(p.paidAt) : "—"}
                    </p>
                  </div>
                  <span className="font-mono text-xs font-bold text-emerald-800 tabular">
